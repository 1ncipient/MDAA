...
public class ViewerType3 implements ViewerCreation{
	...
	public ChartPanel createViewer(AnalysisObject analysis) {
		...
		for (Integer year: dataRec.keySet()) {
			dataset.setValue(dataRec.get(year), labelNames.get(data[0].getDataName()), Integer.toString(year));
		}
		...
		if (count >= 2) {
			...
			for (Integer year: dataRec.keySet()) {
				dataset2.setValue(dataRec.get(year), labelNames.get(data[1].getDataName()), Integer.toString(year));
			}
		...
		if (count == 3) {
			...
			for (Integer year: dataRec.keySet()) {
				dataset.setValue(dataRec.get(year), labelNames.get(data[2].getDataName()), Integer.toString(year));
			}
		}
